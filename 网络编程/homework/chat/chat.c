...
void*recv_msg(void *arg)
{
    ...
    while(1)
    {
        ...
        if(strcmp(p1,"msg")==0)
        {
            char *p2=strtok(NULL,"@");//获取发送方的ip地址
            char *p3=strtok(NULL,"@");//获取发送方的端口号
            int portnum=atoi(p3);
            char *p4=strtok(NULL,"@");//获取发送方的消息内容
            //遍历链表，找到对应客户端，并转发信息给他
            client_node *temp=head;
            while(temp->next!=NULL)
            {
                temp=temp->next;
                if(strcmp(temp->ip,p2)==0&&temp->port==portnum)
                {
                    send(temp->sockfd,p4,strlen(p4),0);
                    break;
